...
struct tag 
{
    ...
    tag() : has(false), add(0), mul(1) {}
    tag(i64 _add, i64 _mul) : has(true), add(_add), mul(_mul) {}
...
void solve()
{
    int n, q;
    cin >> n >> q >> mod;
    SegmentTree<info, tag> segment(n + 1);
    for (int i = 1; i <= n; i++)
    {
        int x;
        cin >> x;
    ...
    while (q--)
    {
        ...
        if (op == 1)
        {
            int x, y, k;
            cin >> x >> y >> k;
            segment.update(x, y, tag(0, k));
        }
        else if (op == 2)
        {
            int x, y, k;
            cin >> x >> y >> k;
            segment.update(x, y, tag(k, 1));
        }
        else 
        {
            int x, y;
            cin >> x >> y;
            cout << segment.query(x, y).sum % mod << "\n";
        }
    }
}
