...
void FindCycle(){
  for (int i=1;i<=n;++i) vis[i]=tag[i]=0;
  sta.clear();cyc.clear();
  // std::cerr<<" ll\n";
  DFSCycle(1,0);
  // std::cerr<<" ll\n";
...
void DFSHDL1(int x,int t){
  ...
  ord[++co]=x;
  dfs[x]=co;
  if (son[x]) DFSHDL1(son[x],t);
  for (int y:e[x])
    if (y!=fa[x]&&y!=son[x]) DFSHDL1(y,y);
}
...
struct tree{
  ...
  tree(int64 _sum,int64 _pre,int64 _suf,int64 _max):sum(_sum),pre(_pre),suf(_suf),max(_max){}
...
void Build(int l,int r,int k){
  if (l==r) return (void)(tr[k]=tree(a[ord[l]]));
  int mid=(l+r)>>1;
  Build(l,mid,k<<1);
  Build(mid+1,r,k<<1|1);
...
void Change(int p,int v,int l,int r,int k){
  if (l==r) return (void)(tr[k]=tree(v));
  int mid=(l+r)>>1;
...
tree Query(int ql,int qr,int l,int r,int k){
  ...
  int mid=(l+r)>>1;
  if (qr<=mid) return Query(ql,qr,l,mid,k<<1);
  if (ql>mid) return Query(ql,qr,mid+1,r,k<<1|1);
  ...
  for (;cq--;){
    ...
    if (o=="C") Change(x,y);
      ...
      if (bel[x]!=bel[y]){
        ...
        ans=std::max(ans,
          (Query(x,cyc.front())+Query(cyc.back(),y)).max);
      }
      // std::cerr<<ans<<'\n';
      xr^=ans;
    }
