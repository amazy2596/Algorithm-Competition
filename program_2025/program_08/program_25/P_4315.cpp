...
struct tag
{
    ...
    tag() : add(0), assign(INF) {}
    tag(i64 add, i64 assign) : add(add), assign(assign) {}

    bool empty() const
    {
        return add == 0 && assign == INF;
    }
...
void solve()
{
    ...
    while (true)   
    {
        ...
        else if (op == "Change")
        {
            int k, w;
            cin >> k >> w;
            auto [u, v, old] = edge[k - 1];
            if (hld.deep[u] > hld.deep[v])
                hld.tree.modify(hld.dfn[u], w);
            else 
                hld.tree.modify(hld.dfn[v], w);
        }
        else if (op == "Cover")
        {
            int u, v, w;
            cin >> u >> v >> w;
            hld.update_path(u, v, tag(0, w));
        }
        else if (op == "Add")
        {
            int u, v, w;
            cin >> u >> v >> w;
            hld.update_path(u, v, tag(w, INF));
        }
        else if (op == "Max")
        {
            int u, v;
            cin >> u >> v;
            cout << hld.query_path(u, v).mx << "\n";
        }
    }
}
