// Problem: [USACO11DEC] Grass Planting G
// URL: https://www.luogu.com.cn/problem/P3038
// Author: amazy
// Date: 2025-08-25 18:08:23
...
struct HLD
{
    ...
    void update_path(int u, int v, const Tag &val)
    {
        ...
        tree.update(l, r, val); // 点权
        // tree.update(l + 1, r, val); // 边权
    }
    ...
    info query_path(int u, int v)
    {
        ...
        int l = min(dfn[u], dfn[v]);
        int r = max(dfn[u], dfn[v]);
        res = res + tree.query(l, r); // 点权
        // res = res + tree.query(l + 1, r); // 边权
    ...
    info query_subtree(int u)
    {
        int l = dfn[u];
        int r = dfn[u] + siz[u] - 1;
        return tree.query(l, r); // 点权
        // return tree.query(l + 1, r); // 边权
    }
};
