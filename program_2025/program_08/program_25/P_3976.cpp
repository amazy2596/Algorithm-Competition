...
info operator+(const info &l, const info &r)
{
    ...
    res.profitL = max({l.profitL, r.profitL, r.mx - l.mn});
    res.profitR = max({l.profitR, r.profitR, l.mx - r.mn});
    ...
    info query_path(int u, int v)
    {
        ...
        while (top[u] != top[v])
        {
            ...
            else 
            {
                int l = dfn[top[v]];
                int r = dfn[v];
                resV = tree.query(l, r) + resV;
                v = fa[top[v]];
            }
        }

        if (dfn[u] > dfn[v])
            resU = tree.query(dfn[v], dfn[u]) + resU;
        else 
            resV = tree.query(dfn[u], dfn[v]) + resV;

        swap(resU.profitL, resU.profitR);
        return resU + resV;
    }
