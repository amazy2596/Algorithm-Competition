...
void solve()
{
    ...
    SegmentTree<info, tagAdd> seg(n + 1);
    for (int i = 1; i <= n; i++)
        seg.modify(i, info(-INF));
    for (int i = 1; i <= n; i++)
    {
        int r = posB[a[i]];
        i64 delta = seg.query(0, r - 1).mx - seg.query(r, r).sum;
        if (delta > 0) seg.update(r, r, delta);
        // i64 mx = seg.query(0, r - 1).mx;
        // i64 cur = seg.query(r, r).mx;
        // if (mx > cur) seg.modify(r, info(mx));
        seg.update(0, r - 1, w[a[i]]);
    }

    i64 ans = seg.query(0, n).mx;
    cout << ans << "\n";
}
