...
void solve()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        while (s1.size() && a[s1.back()] < a[i])
            s1.pop_back();
        pre[i] = s1.empty() ? 0 : s1.back();
        s1.push_back(i);
    }

    int V = 2 * (n + 1);
    vector<int> q(n + 1);
    SegmentTree<info, tagAdd> tree(V);
    for (int i = 1; i <= n; i++)
    {
        q[i] = tree.query(b[i], V).mx;
        tree.modify(a[i], i);
    }
    ...
    for (int i = 1; i <= n; i++)
    {
        i64 l = (b[i] == a[i] ? (i - pre[i]) : 0);
        i64 ll = (a[i] >= b[i] ? pre[i] : min(pre[i], q[i]));
        ans += (l + ll) * (n - i + 1);
    }
