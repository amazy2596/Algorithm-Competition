// Problem: 区间或与与再异或之和最大值
// URL: https://ac.nowcoder.com/acm/contest/117762/F
// Author: amazy
// Date: 2025-10-01 16:34:51
...
struct SegmentTree
{
    ...
    SegmentTree(int _n, Info input) : n(_n)
    {
        init(vector<Info>(n, input));
    }
...
struct block
{
    ...
    {
        return blk_and == o.blk_and && blk_or == o.blk_or;
    }
};
...
void solve()
{
    ...
    vector<block> blks, nxt;  
    SegmentTree<info, tagAdd> dp(n + 1, -inf);
    dp.modify(0, 0);
    for (int i = 1; i <= n; i++)
    {
        nxt.clear();
        ...
        for (auto b : blks)
        {
            int cur_or = b.blk_or | a[i];
            int cur_and = b.blk_and & a[i];
            if (cur_or == nxt.back().blk_or && cur_and == nxt.back().blk_and)
                nxt.back().l = min(nxt.back().l, b.l);
            else 
                nxt.push_back({cur_or, cur_and, b.l, 0});
        }
        ...
        for (auto b : nxt)
        {
            i64 mx = dp.query(b.l - 1, b.r - 1).mx;
            best = max(best, mx + (b.blk_and ^ b.blk_or));
        }
