...
void solve()
{
    ...
    SegmentTree<info, tagAssign> diff(n + 1);
    SegmentTree<info, tagAdd> arr(n + 1);

    for (int i = 1; i <= n; i++)
    {
        arr.update(i, i, tagAdd(a[i]));
        if (i <= n - 1)
            diff.update(i, i, tagAssign(abs(a[i] - a[i + 1])));
    }
    ...
    while (q--)
    {
        ...
        if (op == 1)
        {
            auto res = diff.query(l, r - 1);
            auto L = arr.query(l, l);
            cout << res.sum + L.mx << "\n";
        }
        else 
        {
            arr.update(l, r, tagAdd(1));
            auto x = arr.query(l - 1, l - 1), y = arr.query(l, l);
            diff.update(l - 1, l - 1, tagAssign(abs(x.mx - y.mx)));

            auto t = arr.query(r, r), s = arr.query(r + 1, r + 1);
            diff.update(r, r, tagAssign(abs(t.mx - s.mx)));
        }
    }
}
