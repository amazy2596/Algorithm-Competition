...
void dft(vector<int> &a)
{
    ...
    if (roots.size() < n) 
    {
        ...
        while ((1 << k) < n) 
        {
            int e = fast_pow(G, (mod - 1) / (1ll << (k + 1)));
...
void solve()
{
    ...
    auto calc = [&](auto self, int l, int r)
    {
        ...
        vector<int> tmp_f(f.begin() + l, f.begin() + mid + 1);
        vector<int> tmp_g(g.begin(), g.begin() + r - l + 1);
        vector<int> tmp = mul(tmp_f, tmp_g);
        for (int i = 0; i < tmp.size(); i++)
        {
            int idx = i + l;
            if (mid + 1 <= idx && idx <= r)
                f[idx] = (f[idx] + tmp[i]) % mod;
        }
...
//     NTT(int _mod, int _g) : mod(_mod), g(_g) 
//     {
//         roots = {0, 1};
//     }
...
//         __int128_t c123 = c12 + (__int128_t)k2 * (P1 % P) * (P2 % P);
