...
BigInt BigInt::operator / (const BigInt& other) const
{
    ...
    if (this->get_abs() < other.get_abs()) return 0;
    
    BigInt res, current; res.sign = sign * other.sign;
    res.s.resize(s.size());
    BigInt abs_other = other.get_abs();

    for (int i = s.size() - 1; i >= 0; --i)
    {
        current.s.insert(current.s.begin(), s[i]);
        current.normalize();
        int x = 0;
        while (current >= abs_other) { current -= abs_other; x++; }
        res.s[i] = x;
    }
    res.normalize(); return res;
}
...
void solve()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        vector<vector<BigInt>> dp(m + 1, vector<BigInt>(m + 1));
        for (int j = 1; j <= m; j++)
            dp[j][j] = a[i][j];
        for (int len = 2; len <= m; len++)
        {
            ...
            {
                int r = l + len - 1;
                dp[l][r] = max({dp[l][r], 2 * dp[l + 1][r] + a[i][l], 2 * dp[l][r - 1] + a[i][r]});
            }
        }
